...
const Login = () => {
  ...
  const onSubmitHandler = async (e: any) => {
    ...
    try {
      if (currentState === 'Sign Up') {
        const response = await axios.post(`${Server_Url}/user/register`, { name, email, password })
  ...
  return (
    <form onSubmit={onSubmitHandler} className="flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-[#FFFF]" >
      ...
      <input onChange={(e) => setPassword(e.target.value)} value={password} className="w-full px-3 py-2 border border-gray-800 " type="password" placeholder="Password" />
